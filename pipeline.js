...
**核心观点**：用 1-3 句话点破本集最根本的论断、判断或结论。不是"本集讨论了 X"，而是"关于 X，嘉宾的核心论断是 Y"。

**阐述逻辑**：用 4-8 句话梳理本集的论证链条，用"因为 A，所以 B；进一步地因为 B，所以 C"的因果结构表达。不要简单罗列章节或话题，而是抽象出推理骨架：前提 → 中间论点 → 结论。

**叙述脉络**：用 1-2 句话描述嘉宾/内容的展开结构（如：现象切入→机制拆解→行业预判；时间纵轴→当下横切→未来展望；问题提出→多角度论证→反常识收尾）。

**要求**：
- 总长度 300-500 字
- 是逻辑的抽象，不是摘要的复述
- 不要用"本文讨论了"这种套话，直接给判断和结构
...
**只提取嘉宾在本集实际使用或讨论的概念**。不要从"常见概念库"里照搬术语来凑数——如果嘉宾没说「马太效应」就绝对不要加进去。

### 提取范围（只作为分类参考）

只有当以下类型的概念**在本集逐字稿中实际出现**时，才提取：
- 嘉宾自创或反复使用的概念、框架、比喻（最有价值，标注「嘉宾定义」）
- 嘉宾明确引用的理论/效应（说出了具体名字）
- 嘉宾用作论据的行业术语（实际在论证中使用）
- 嘉宾引用的历史人物/学派（且展开过讨论）

### 验证步骤（每个术语都要通过）

提取前，先确认：
1. 嘉宾在逐字稿里**具体哪一段**说到这个概念？
2. 他是在用它论证什么？不是顺口一提而是实际使用
3. 如果删掉这个定义，读者理解嘉宾观点是否有障碍？
...
**[术语名]**（English Name）
[出处/学科来源]。[通俗解释，2-3句话，说清楚它是什么、为什么重要、嘉宾在本集中如何使用]

**其他要求**：
- **宁少勿滥**：8-20 个为宜；如果是纯访谈/对话，能提 5-10 个好概念就很好
- 有英文名的一定要标注
- 定义要准确、简洁、可独立阅读
- 嘉宾自创概念一定要标「嘉宾定义」
